...
public class Main {
    ...
    static void play(Scanner scanner, Random random) {
        System.out.println("Напишите длину поля,что будет > 2:");
    ...
    static boolean isFinal(char[][] field, char sign, int size) {
        if (isWin(field, sign, size)) {
            String name = sign == 'X' ? "Настенька" : "Робот";
            System.out.println(String.format("%s победитель", name));
            return true;
        }
        if (isDraw(field)) {
            System.out.println("Ничья - лучше, чем проигрыш :)");
            return true;
        }
        return false;
    }
    ...
    static void doAIMove(Random random, char[][] field) {
        ...
        for (int i = 0; i < field.length - 2; i++){
            for (int j = 0; j < field.length - 2; j++){
                if (field[i][j] == 'X' && field[i][j+1] == 'X' && field[i][j+2] == '-'){
                    field[i][j+2] = 'O';
